...
const ChatList = ({ selectedChatId, setSelectedChatId }) => {
  ...
  const [createChat, { loading: creatingChat }] = useMutation(
    CREATE_CHAT_MUTATION, { refetchQueries: ['GetChats'] }
  );
        ...
        {data.chats.map((chat) => (
          <div
            ...
            className={`p-3 rounded-md cursor-pointer transition-colors ${
              selectedChatId === chat.id ? 'bg-purple-100' : 'hover:bg-gray-100'
            }`}
          >
            {/* This now shows the title, or a default name if no title exists yet */}
